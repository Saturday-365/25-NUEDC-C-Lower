...
#define can1_txd() HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1, &txMsg, tx_data)
#define can2_txd() HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan2, &txMsg, tx_data)
...
{
  txMsg.Identifier = 0x12345678;
  txMsg.IdType = FDCAN_EXTENDED_ID;																// 标准ID 
  txMsg.TxFrameType = FDCAN_DATA_FRAME;														// 数据帧 												// 发送数据长度 
  txMsg.DataLength = FDCAN_DLC_BYTES_8;//FDCAN_DLC_BYTES_8 ;           //配置CAN发送：数据长度
  txMsg.ErrorStateIndicator = FDCAN_ESI_ACTIVE;										// 设置错误状态指示 								
  txMsg.BitRateSwitch = FDCAN_BRS_OFF;															// 不开启可变波特率 
  txMsg.FDFormat = FDCAN_CLASSIC_CAN;															// 普通CAN格式 
  txMsg.TxEventFifoControl = FDCAN_NO_TX_EVENTS;										// 用于发送事件FIFO控制, 不存储 
  txMsg.MessageMarker = 0x00; 			// 用于复制到TX EVENT FIFO的消息Maker来识别消息状态，范围0到0xFF                
...
{
	uint8_t tx_data[8]= {0};
	txMsg.Identifier = Communication_Type_SetSingleParameter<<24|Master_CAN_ID<<8|Motor->CAN_ID;
